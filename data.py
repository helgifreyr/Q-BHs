...
def get_data(ms,ws,branch,rhs):
    string = 'm='+ms+'/w='+ws+'/'+branch+'/rh='+rhs+'/'
...
def get_Bstar_data(ms,ws,branch):
    string = 'solitonic-limit/m='+ms+'/'+branch+'/w='+ws+'/'
    dirs = sorted(glob.glob(string))
...
def get_extremal_data(ms,ws,branch,rhs):
    string = 'extremal/m='+ms+'/w='+ws+'/'+branch+'/rh='+rhs+'/'
    dirs = sorted(glob.glob(string))
    datas = scipy.zeros((len(dirs),30))
    i=0
    for dir in dirs:
